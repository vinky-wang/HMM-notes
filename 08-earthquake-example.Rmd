...
The forecast distribution from the above can be modified to compute the bivariate forecast distribution. 
...
```{r}
eq_forecast <- data.frame(pois.HMM.forecast_bivariate(1:10, 1:10, x, eq_init))
...
pois.HMM.stanfit <- stan(model_code = poisHMM, data = stan_data, refresh=2000)
```
...
```{r}
# Plot of the posterior draws for the lambda parameters
