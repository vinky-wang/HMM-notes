...
plot(eq_dat, type = "b", pch=19, cex=0.5, xlab="Year", ylab="Count", main="")
```

```{r e2, fig.cap="Histogram of major earthquakes overlaid with a Poisson density of mean equal to the observed counts.", echo=F}
{hist(eq_dat$Count, freq=FALSE, breaks=100, main="", xlab="Count")
points(0:max(eq_dat$Count), dpois(0:max(eq_dat$Count), mean(eq_dat$Count)),
      col="hotpink", pch=16)
}
...
data.frame(w2n(nlm(mllk, wpar, eq_dat$Count)$estimate)) %>%
...
```{r forecastfour, fig.cap="Forecast probabilities for the fitted three-state Poisson-HMM (non-stationary) for 1 to 4 years ahead.", echo=F}
h<-4
xf<-0:45
forecasts<-pois.HMM.forecast(xf,h,eq_dat$Count,mod3h)
...
```{r local, fig.cap="Local decoding for the three-state Poisson-HMM (non-stationary). The horizontal lines indicate the state-dependent means and the points indicate the decoded state at the given time.", echo=F}
...
```{r comparelocal, fig.cap="Local decoding for the three-state Poisson-HMM (non-stationary). The horizontal lines indicate the state-dependent means and the points indicate the decoded state at the given time using  pois.HMM.local_decoding (purple) and momentuHMM (orange).", echo=F}
...
```{r global, fig.cap="Global decoding for the three-state Poisson-HMM (non-stationary).", echo=F}
...
```{r comparelg, fig.cap="Global (green) and local (brown) decoding for the three-state Poisson-HMM (non-stationary).", echo=F}
{plot(eq_dat, type = "l", xlab="", ylab="Count", main="")
abline(h=10, col="blue")
abline(h=15, col="blue")
abline(h=30, col="blue")
...
```{r postg, fig.cap="Posterior distribution for the transition probabilities of the three-state Poisson-HMM.", echo=F}
post_Gamma <- rstan::extract(pois.HMM.stanfit, pars=c("Gamma"))

for(i in 1:3){
  for(j in 1:3){
    hist(post_Gamma[[1]][1:1000, i, j], main="", xlab=substitute(gamma[i][j], list(i = i, j = j)))
  }
}
